...
# upstream (o-tiling: github.com/oliwebd/o-tiling).
#
...
OT_SCHEMAS="$HOME/.local/share/gnome-shell/extensions/$OT_UUID/schemas"

{
  ...
  journalctl --since "30 min ago" _COMM=gnome-shell --no-pager 2>/dev/null \
    | grep -iE "JS ERROR|o-tiling|disposed|exception" | tail -30
  echo
  ...
  echo "===== [5] o-tiling: estado y uptime de gnome-shell ====="
  [[ -d "$OT_SCHEMAS" ]] && export GSETTINGS_SCHEMA_DIR="$OT_SCHEMAS"
  echo -n "tile-by-default  : "; gsettings get org.gnome.shell.extensions.o-tiling tile-by-default 2>&1
  echo -n "o-tiling activa : "; gnome-extensions list --enabled 2>/dev/null | grep -qx "$OT_UUID" && echo "sí" || echo "NO"
